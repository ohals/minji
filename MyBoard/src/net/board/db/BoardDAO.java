...
public class BoardDAO {
	...
	public List getBoardList(int page,int limit){
		...
		String board_list_sql= "select * from (select @num:=@num+1 rnum,BOARD_NUM,BOARD_NAME,BOARD_SUBJECT, BOARD_CONTENT,BOARD_FILE,BOARD_RE_REF,BOARD_RE_LEV, BOARD_RE_SEQ,BOARD_READCOUNT,BOARD_DATE from (select * from board order by BOARD_RE_REF desc,BOARD_RE_SEQ asc)a, (select @num:=0)b )c where c.rnum>=? and c.rnum<=?";
